...
parse_repos_file() {
    ...
    while IFS= read -r line || [[ -n "$line" ]]; do
        ...
        if [[ "$line" =~ ^[a-zA-Z0-9_/-]+: ]]; then
            current_repo="${line%:}"
            echo "----------------------------------------"
            echo "Destination directory: $current_repo"
...
parse_repos_file
echo "----------------------------------------"
echo "Finished parsing .repos file."
...
for repo_name in "${!repo_urls[@]}"; do
    ...
    if [[ ${#filtered_subpaths[@]} -eq 0 ]]; then
        echo "No subpaths found, so entirely cloning $url into $repo_name"
        # 여기에 예외처리 추가
        if [ -d "$repo_name" ] && [ -n "$(ls -A "$repo_name" 2>/dev/null)" ]; then
            echo "⚠️  Skipping: '$repo_name' already exists and is not empty"
        else
            git clone --branch "$version" "$url" "$repo_name"
            rm -rf "$repo_name/.git"
        fi
        continue
    fi
