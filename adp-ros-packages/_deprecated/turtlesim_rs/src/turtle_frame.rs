...
use rclrs::{CreateBasicExecutor, Node, OptionalParameter, Service, Time, SpinOptions};

use std_srvs::srv::Empty;
use turtlesim_rs_msgs::srv::{Kill, Spawn};
...
pub struct TurtleFrame<'a> {
    ...
    node: Node,
    last_turtle_update: Time,
    
    bg_r_param: OptionalParameter<i64>,
    bg_g_param: OptionalParameter<i64>,
    bg_b_param: OptionalParameter<i64>,

    srv_rx: Receiver<ServiceMsg>,

    clear_srv: Service<Empty>,
    kill_srv: Service<Kill>,
    reset_srv: Service<Empty>,
    spawn_srv: Service<Spawn>,
}
...
impl<'a> TurtleFrame<'a> {
    pub fn new(ctx: egui::Context) -> Self {
        ...
        let node = executor.create_node("turtlesim_rs").unwrap();
        println!("Starting turtlesim_rs with node name {}", node.name());

        let last_turtle_update = node.get_clock().now();

        let bg_r_param = node
            .declare_parameter::<i64>("background_r")
            .default(BACKGROUND_R as i64)
            .optional()
            .unwrap();

        let bg_g_param = node
            .declare_parameter::<i64>("background_g")
            .default(BACKGROUND_G as i64)
            .optional()
            .unwrap();

        let bg_b_param = node
            .declare_parameter::<i64>("background_b")
        ...
        let kill_srv = node
            .create_service(
                "kill",
                move |req: turtlesim_rs_msgs::srv::Kill_Request| {
        ...
        let spawn_srv = node
            .create_service(
                "spawn",
                move |req: turtlesim_rs_msgs::srv::Spawn_Request| {
        ...
        //     if let Some(node_clone) = node_weak.upgrade() {
        //         let _ = executor::spin_once(node_clone, Some(time::Duration::ZERO));
        //     } else {
        //         break;
        //     }
        // });
    ...
    pub fn spawn(&mut self, name: &str, x: f32, y: f32, angle: f32) -> String {
        ...
        let rand_index = rng.gen_range(0..self.turtle_images.len());
        let turtle_img = self.turtle_images[rand_index].clone();
