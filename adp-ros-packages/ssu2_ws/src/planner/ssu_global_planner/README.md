...
-   전체 자율주행 시스템 중 **글로벌 플래너 파트 단독 담당**
-   Autoware Mission Planner **아이디어 기반해** 전역 경로 생성 전 과정
    구현
...
| 토픽 이름 | 메시지 타입 | 설명 |
|-----------|-------------|------|
| `/global_waypoints` | nav_msgs/Path | 최단 전역 주행 경로 |
...
-   사용 파일 경로: ssu2_ws/assets/sample-map-planning/lanelet2_map.osm
...
public:
  ...
  using Edge     = std::pair<NodeId, double>;   // (neighbor id, distance[m])
...
private:
  ...
  // OSM 노드 ID → (이웃 노드 ID, 거리[m]) 인접 리스트
...
-   **nodes\_**
    -   Key: OSM 노드 ID\
    -   Value: 해당 노드의 위경도 좌표 (lat, lon)
-   **edges\_**
    -   Key: 기준 노드 ID\
    -   Value: (이웃 노드 ID, 거리\[m\]) 쌍의 리스트

### ✅ 거리 계산 방식

-   OSM에 저장된 위경도 좌표를 이용하여\
    **Haversine 공식 기반 거리(m) 계산**
-   계산된 거리는 `Edge`의 두 번째 값으로 저장됨
-   이 값을 기준으로 Dijkstra 최단 경로 탐색 수행

------------------------------------------------------------------------

### OSM 노드를 정점으로, 노드 간 실제 거리(m)를 가중치로 갖는 ID 기반 인접 리스트 그래프로 구성
