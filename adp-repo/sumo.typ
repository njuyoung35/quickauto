...
- \<2D-BOUNDING_BOX\> : (\<FLOAT\>, \<FLOAT\>, \<FLOAT\>, \<FLOAT\>)
- \<PROJ_DEFINITION\>

=== referenced file types

- \<NETWORK_FILE\> : SUMO network file (netgenerate 또는 netconvert로 생성된)
- \<ROUTES_FILE\> : SMO routes file (duarouter, jtrrouter, 또는 손으로 생성된)
- \<TYPE_FILE\> : SUMO edge type file
- \<OSM_FILE\> : OpenStreetMap file
...
xml 파일 내에서 ```xml <net-file value="${NETFILENAME}.net.xml"/>``` 이와 같이 환경변수 값을 포함시키도록 할 수 있다.
...
.net.xml 형식이며, 지도를 표현하고, net_file.xsd에 스키마가 정의되어 있다.
...
SUMO's Road Intersection Model https://elib.dlr.de/93669/1/LNCS_SUMOIntersections.pdf 참고

intersection(교차로) 안에 internal edge라는 가상의 좌회전/직진/우회전 등 라인이 있다고 가정한다. 교차로는 node(junction)으로만 단순히 표현하기에는 시뮬레이션 상 어려움이 있었다. 그래서 교차로에 인접하는 차선들을 a_0, a_1, a_2, .., b_0, b_1, b_2, .., 와 같이 라벨링 하고 이 때 -c_0와 같이 음수 표현으로 outgoing lane을 표현한다. 그리고 차량이 가질 수 있는 궤적을 internal edge로 표현하는 것이다.
...
자세한 것은 https://sumo.dlr.de/docs/Simulation/Traffic_Lights.html \
TLS-programs
...
- '1_f_0'은 '1_v_0'을 통해 '1_t_0'으로 연결된다.
- '[from=1_f_0, to=1_t_0 via=1_v_0]'와 '[from=1_v_0 to=1_t_0]'이 둘 다 존재해야 한다? 후속 커넥션도 묘사해야 하는 것으로 보임.
- 또는 '[from=1_f_0 to=1_v_0]' 같이 불필요한 커넥션을 추가해도 안됨.
...
대부분 xml 파일들에 대하여 xsd 명세가 있으므로 https://sumo.dlr.de/docs/Other/File_Extensions.html 방문해서 살펴보도록 하자. \
참고로 xsd 파일들은 \<SUMO_HOME\>/data/xsd 디렉토리에 있다. \
...
- output files
  ...
  - 참고로 실험적으로나마 csv, Parquet 형식의 tabular output을 배노내는 옵션도 존재하는 것으로 보인다 `--fcd-output` 옵션을 사용하면 csv나 parquet 형식으로 내보낼 수 있다. https://sumo.dlr.de/docs/TabularOutputs.html 에서 더 살펴보도록 하자.
- other files
  - \*.xml (edge diff)
...
```xml <xsd:element name="net" type="netType">```로 구성된다.
...
- stopOffsetType
- connectionType
  - [attr] from: xsd:string // required
  - [attr] to: xsd:string // required
  - [attr] fromLane: xsd:string // required
  - [attr] toLane: xsd:string // required
  - [attr] pass: boolType // optional
  - [attr] keepClear: boolType // optional
  - [attr] centPos: floatType // optional
  - [attr] visibility: floatType // optional
- prohibitionType
  - [attr] prohibitor: xsd:string // required
  - [attr] prohibited: xsd:string // required
- roundaboutType
  - [attr] nodes: xsd:string // required
  - [attr] edges: xsd:string // required
- neighType
  - [attr] lane: xsd:string // required